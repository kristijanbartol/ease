...
class GarmentSimulator:
    ...
    def _apply_edge_constraints_vectorized(self):
        """Enhanced edge constraint application with adaptive correction."""
        v1 = self.garment_vertices[self.garment_edges[:, 0]]
        v2 = self.garment_vertices[self.garment_edges[:, 1]]
        
        edge_vectors = v2 - v1
        lengths = np.linalg.norm(edge_vectors, axis=1)
        mask = lengths > self.target_length
        
        if np.any(mask):
            masked_edges = self.garment_edges[mask]
            
            # Calculate violation severity
            length_violations = lengths[mask] - self.target_length
            max_violation = np.max(length_violations)
            
            # Adapt correction factor based on violation severity
            correction_factor = np.clip(
                length_violations / max_violation * self.max_correction_factor,
                self.min_correction_factor,
                self.max_correction_factor
            )
            
            # Apply stronger corrections for more severe violations
            length_ratios = (lengths[mask] - self.target_length) / lengths[mask]
            masked_corrections = edge_vectors[mask] * length_ratios[:, np.newaxis]
            
            # Scale corrections by adaptive factor
            ...
            if np.any(mask):
                ...
                max_allowed_correction = self.target_length * np.maximum(
                    ...
                    np.minimum(0.2, np.max(length_violations) / self.target_length)  # Adaptive limit
                )

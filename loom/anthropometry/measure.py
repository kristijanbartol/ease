...
class Measurer():
    ...
    def measure(self, 
        ...
        self.measurements['weight'] = trimesh.Trimesh(self.verts, self.faces).volume * 1000.
...
class MeasureSMPL(Measurer):
    ...
    def __init__(self, project_dir, smpl_dir):
        super().__init__()
        
        self.model_type = "smpl"
        #self.body_model_root = 'data/'
        self.smpl_dir = smpl_dir
        self.body_model_path = os.path.join(smpl_dir, 
                                            self.model_type)
    ...
    def from_body_model(self,
        ...
        #                     num_thetas=self.num_joints)    
        model = SMPL(
            model_path=os.path.join(self.smpl_dir, f'SMPL_{gender.upper()}.pkl'), 
            gender=gender
        )
